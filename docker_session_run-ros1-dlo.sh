...
IMAGE_NAME='dlo_noetic'
TMUX_SESSION='ros1_session'

DATASET_CONTAINER_PATH='/ros_ws/dataset/recorded-dlo.bag'
...
roslaunch direct_lidar_odometry dlo.launch pointcloud_topic:='"$POINTCLOUD_TOPIC"' imu_topic:='"$IMU_TOPIC"' use_sim_time:=true
'\'' C-m
...
source /ros_ws/devel/setup.bash
echo "[record] start"
rosbag record /robot/dlo/odom_node/odom /robot/dlo/odom_node/pointcloud/keyframe -O '"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME"'
echo "[record] exit"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    source /opt/ros/noetic/setup.bash
    source /ros_ws/devel/setup.bash
    rosrun dlo-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-dlo\"
  "
