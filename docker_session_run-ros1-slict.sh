...
IMAGE_NAME='slict_noetic'
TMUX_SESSION='ros1_session'

DATASET_CONTAINER_PATH='/ros_ws/dataset/recorded-slict.bag'
BAG_OUTPUT_CONTAINER='/ros_ws/recordings'

RECORDED_BAG_NAME="recorded-slict.bag"
...
export USER=ros
roslaunch slict run_mcdviral.launch  use_sim_time:=true  use_prior_map:=0 bag_file:=/ros_ws/dataset/recorded-slict.bag config_file:=/ros_ws/src/slict/config/mcdviral_livox_hhs.yaml prior_map_dir:=/tmp; tmux wait-for -S BAG_DONE;
'\'' C-m
...
source /ros_ws/devel/setup.bash
echo "[record] start"
rosbag record /kfcloud /opt_odom -O '"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME"'
echo "[record] exit"
'\'' C-m
...
docker run -it --rm \
  ...
  /bin/bash -c "
    ...
    source /opt/ros/noetic/setup.bash
    source /ros_ws/devel/setup.bash
    rosrun slict-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-slict\"
  "
